...
void UVAT_Retarget::OnRebuildAnimations(UAnimSequence* InAnimSequence)
{
...
#if ENGINE_MAJOR_VERSION > 4
	IAnimationDataController& theAnimController = InAnimSequence->GetController();
	theAnimController.SetPlayLength(theLength);
	theAnimController.Resize(theLength, 0, theLength);

	const int32 theFrameRate = 1.f / UVAT_Library::GetFrameTime(InAnimSequence);
	FFrameRate TargetFrameRate(theFrameRate, 1);
	...
	for (TPair<FName, FVirtualBoneCurvesData>& theTrackPair : theBoneLSDataMap)
	{
		...
		for (int32 KeyIndex = 0; KeyIndex < theNumberOfKeys; KeyIndex++)
		{
			...
			// Cache to bone track data
			UVAT_Library::AddBoneTrackKey(theBoneTrackData, theTargetTransformCS);
		}
